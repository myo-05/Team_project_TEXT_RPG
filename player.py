...
class player(status.MainStatus):
    ...
    def give_message(self,s):
        #상태 메시지 출력을 만들어서 인자값으로 보낸다! 
        string = f" 체력: {self.bar(self.max_hp,self.now_hp)}\n 마나: {self.bar(self.max_mp,self.now_mp)}\n EXP : {self.bar(self.max_experience,self.now_experience)}\n이름 : {self.name} | 체력 : {self.now_hp}/{self.max_hp} | 마나 : {self.now_mp}/{self.max_mp}\n\n{s}"
    ...
    def get_exp(self,target):
        ...
        self.now_experience += target.max_experience
        while self.now_experience >= self.max_experience:
            #현재 경험치가, 최대 경험치보다 작을때까지
            self.now_experience -= self.max_experience
            self.level_up()
...
########################################################################################################################################################
    def magic_attack_massage(self): # self 넣어야함 
        ...
        return "머글이, 근육을 꿈틀거리며 노려본다. [ 공격 스킬 ] : " + random.choice(text)
    ...
    def cure_massage(self):
        ...
        return " 근육들이 회복 중에 있습니다. [ 회복 스킬 ] 머글쓰 : " + random.choice(text)
    
    def counter_massage(self):
        text = [ ["샷건 쏘겠쓰"],
            ["형은 말하지 않아도, 다~ 아는 방법이 있다?"],
            ["전변호사 처리해! 지지지직"]
        ]
        return " 진실의 방으로 입장하셨습니다 !! [ 필살기! ]: " + random.choice(text)
#############################################################################################################################################################
...
########################################################################################################################################################
    def magic_attack_massage(self): # self 넣어야함 
        ...
        return "피시방에 잼민이의 목소리가 우렁차게 울립니다! [ 공격 스킬 ] : " + random.choice(text)

    def cure_massage(self):
        text = [ ["맘스터콜"], #대규모 맘카페 엄마들의 댓글테러를 말함
            ["구몬 읽기"],
            ["후원하고 환불해달라 때쓰기"]
        ]
        return "조카들이 당신의 방에 쳐들어왔습니다! [ 회복 스킬 ] : " + random.choice(text)
    ...
    def counter_massage(self):
        ...
        return "내가 한거 아니라고 !!! 빼애애액 ! [ 필살기! ]: " + random.choice(text)
#############################################################################################################################################################
...
########################################################################################################################################################
    def magic_attack_massage(self): # self 넣어야함 
        text = [ ["부엉이가 물에 빠지면? 첨부엉 읔ㅋㅋㅋㅋㅋㅋㅋㅋ"],
    ...
    def cure_massage(self):
        text = [ ["커피 한 잔 씩 해야지?"], #대규모 맘카페 엄마들의 댓글테러를 말함
            ["라떼는 말이야?"],
            ["손대리 오늘 회식있는거 알지?"]
        ]
        return "부장님이 집에 들어가기 싫어합니다!! [ 회복 스킬 ] : " + random.choice(text)
    
    def counter_massage(self):
        text = [ ["먼저 퇴근들해~(단호,하지만 눈치가 보인다..)"],
            ["이걸 보고서라고 써온거야?"],
            ["요즘 MZ은 말이야?!"]
        ]
        return "5년차가 이것도 못해?! (부장님이 화나셔서 두피도 빨개졌습니다.)[ 필살기! ]: " + random.choice(text)
#############################################################################################################################################################
...
########################################################################################################################################################
    def magic_attack_massage(self): # self 넣어야함 
        text = [ ["도넛 발싸!!!!!!!"],
            ["가스 가스 가스!! 화생방 경보!"],
            ["술 자리에, 당신만 남겨두고 담배피러 갑니다!"]
        ]
        return "스모커가 연기를 당신에게 뿜습니다! [ 공격 스킬 ] : " + random.choice(text)

    def cure_massage(self):
        text = [ ["무호흡 흡연"], #대규모 맘카페 엄마들의 댓글테러를 말함
            ["전자 담배를 충전합니도"],
            ["스모커가 당신의 돛대를 뺏어갑니다!"]
        ]
        return "새해에는 금연 할려 했는데...[ 회복 스킬 ] : " + random.choice(text)
    ...
    def counter_massage(self):
        ...
        return "난 담배는 피워도 바람은 안피워[ 필살기! ]: " + random.choice(text)
#############################################################################################################################################################
